...
<div class="d-flex align-items-center gap-3 mb-4">
    ...
    <h2 class="fw-bold m-0">Edit Task Assignment</h2>
</div>

<div class="card shadow-sm">
    <div class="card-body">
        <form action="{{ route('task_assignments.update', $taskAssignment) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-3">
                ...
                <select name="task_id" id="task_id" class="form-select" required>
                    ...
                    @foreach($tasks as $task)
                        <option value="{{ $task->id }}" {{ $task->id == $taskAssignment->task_id ? 'selected' : '' }}>
            ...
            <div class="mb-4">
                ...
                <select name="employee_id" id="employee_id" class="form-select" required>
                    ...
                    @foreach($employees as $employee)
                        <option value="{{ $employee->id }}" {{ $employee->id == $taskAssignment->employee_id ? 'selected' : '' }}>
